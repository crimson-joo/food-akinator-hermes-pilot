...
- Character rendering is now behind `src/ui/character/CharacterStage.ts`, `characterRuntime.ts`, `characterAssetManifest.ts`, and `cssFallbackRuntime.ts`. The manifest keeps `preferredRuntime: 'rive'` and the adapter can select manifest-valid `rive`, secondary `lottie`, or playable `css-fallback`. Missing assets expose `data-runtime-status="fallback"`; malformed available Rive/Lottie manifests fail closed to CSS with `data-runtime-status="failed"` and `data-runtime-reason`, so the app cannot silently claim production runtime readiness.
