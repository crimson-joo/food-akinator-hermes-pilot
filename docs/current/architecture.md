...
- Character rendering is now behind `src/ui/character/CharacterStage.ts`, `characterRuntime.ts`, `characterAssetManifest.ts`, and `cssFallbackRuntime.ts`. The manifest keeps `preferredRuntime: 'rive'` but selects `css-fallback` while authored Rive/Lottie assets are missing, preserving the production-blocked runtime claim honestly.
