...
import { renderCharacterStage } from './character/CharacterStage.js';
import { planAnswerMotion, planWrongRecoveryMotion, runMotionPlan, type ScheduledUiEvent } from './motionScheduler.js';
...
<main ${attrs.join(' ')}>
  ...
  ${renderCharacterStage(model.lastAnswer ? { cue, lastAnswer: model.lastAnswer } : { cue })}
...
export function mountApp(root: HTMLElement): void {
  ...
  const schedule = (handler: (event: ScheduledUiEvent) => void, plan: ReturnType<typeof planAnswerMotion> | ReturnType<typeof planWrongRecoveryMotion>) => {
    runMotionPlan(window, handler, plan);
  };
  ...
  root.addEventListener('click', (event) => {
    ...
    try {
      ...
      if (button.dataset.action === 'reject-guess' && model.session?.guess) {
        ...
        const handleRecoveryEvent = (scheduledEvent: ScheduledUiEvent) => {
          if (scheduledEvent.type === 'advanceRecoveryBeatFromGuess') {
            dispatch({
              type: 'advanceRecoveryBeat',
              beat: scheduledEvent.beat,
              session: submitGuessFeedback(guessingSession, { candidateId: scheduledEvent.candidateId, accepted: false }, foodKnowledgeBase),
            });
            return;
          }
          if (scheduledEvent.type === 'completeRecovery') {
            if (model.phase === 'recovering' && model.recoveryBeat === 'refocus' && model.session) dispatch({ type: 'recovered', session: model.session });
            return;
          }
          dispatch(scheduledEvent as UiEvent);
        };
        schedule(handleRecoveryEvent, planWrongRecoveryMotion({ candidateId: candidate.id, candidateName: candidate.nameKo }));
        return;
      }
      ...
      if (answer && model.session?.currentQuestion) {
        ...
        const answeringSession = model.session;
        const handleAnswerEvent = (scheduledEvent: ScheduledUiEvent) => {
          if (scheduledEvent.type === 'engineReadyFromAnswer') {
            dispatch({ type: 'engineReady', session: submitAnswer(answeringSession, { questionId: scheduledEvent.questionId, answer: scheduledEvent.answer }, foodKnowledgeBase) });
            return;
          }
          dispatch(scheduledEvent as UiEvent);
        };
        schedule(handleAnswerEvent, planAnswerMotion({ questionId, answer }));
      }
    } catch (error) {
