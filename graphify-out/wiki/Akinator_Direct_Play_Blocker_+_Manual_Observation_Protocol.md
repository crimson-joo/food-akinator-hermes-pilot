# Akinator Direct Play Blocker + Manual Observation Protocol

> God node · 9 connections · `docs/research/akinator-direct-play-blocker-and-protocol.md`

**Community:** [[Community 4]]

## Connections by Relation

### contains
- [[시도한 경로]] `EXTRACTED`
- [[필요한 사용자 입력/권한]] `EXTRACTED`
- [[Manual play protocol]] `EXTRACTED`
- [[akinator-direct-play-blocker-and-protocol.md]] `EXTRACTED`
- [[현재 판정]] `EXTRACTED`
- [[하지 않은 것]] `EXTRACTED`
- [[왜 blocker인가]] `EXTRACTED`
- [[Observation checklist]] `EXTRACTED`
- [[Builder gate]] `EXTRACTED`

---

*Part of the graphify knowledge wiki. See [[index]] to navigate.*