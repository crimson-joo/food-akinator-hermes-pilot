# Community 15

> 12 nodes

## Key Concepts

- **Release — Zero-to-One** (11 connections) — `docs/current/release.md`
- **release.md** (1 connections) — `docs/current/release.md`
- **현재 release 상태** (1 connections) — `docs/current/release.md`
- **Release policy** (1 connections) — `docs/current/release.md`
- **Release automation contract** (1 connections) — `docs/current/release.md`
- **Repository artifact policy** (1 connections) — `docs/current/release.md`
- **금지** (1 connections) — `docs/current/release.md`
- **배포 완료 정의** (1 connections) — `docs/current/release.md`
- **배포 완료 정의** (1 connections) — `docs/current/release.md`
- **배포 완료 정의** (1 connections) — `docs/current/release.md`
- **배포 완료 정의** (1 connections) — `docs/current/release.md`
- **배포 완료 정의** (1 connections) — `docs/current/release.md`

## Relationships

- No strong cross-community connections detected

## Source Files

- `docs/current/release.md`

## Audit Trail

- EXTRACTED: 22 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [[index]] to navigate.*