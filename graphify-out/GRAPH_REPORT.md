...
## Corpus Check
- 29 files · ~16,850 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 396 nodes · 446 edges · 27 communities (25 shown, 2 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `67b6a4c9`
...
6. `Akinator Direct Play Synthesis — Builder Handoff` - 9 edges
7. `Akinator Direct Play Blocker + Manual Observation Protocol` - 9 edges
8. `Product — Food Akinator Zero-to-One` - 9 edges
9. `Candidate` - 8 edges
10. `QA — Zero-to-One Acceptance` - 8 edges
...
- `buildNextSession()` --calls--> `scoreCandidates()`  [EXTRACTED]
  src/engine/session.ts → src/engine/scoring.ts
- `pickNextQuestion()` --calls--> `selectNextQuestion()`  [EXTRACTED]
  src/engine/session.ts → src/engine/selector.ts
...
Cohesion: 0.05
Nodes (53): ANSWER_VALUES, AnswerKey, AnswerValue, asRecord(), Candidate, LEVELS, Question, QUESTION_AXES (+45 more)
...
Cohesion: 0.15
Nodes (19): ANSWER_OPTIONS, answerOptions(), askingSession(), BuildInput, buildNextSession(), CharacterCue, DEFAULT_POLICY, EngineSession (+11 more)
...
## Knowledge Gaps
- **260 isolated node(s):** `isExplicitHermesRun`, `answerOptions`, `coreQuestions`, `coreCandidates`, `session` (+255 more)
...
- **Why does `작은 음식 도메인 Akinator-like 추론 엔진 + 캐릭터/모션 파이프라인 제안` connect `Community 1` to `Community 18`, `Community 10`, `Community 13`?**
  _High betweenness centrality (0.033) - this node is a cross-community bridge._
- **Why does `7. 엔진 API 초안` connect `Community 10` to `Community 1`?**
  _High betweenness centrality (0.010) - this node is a cross-community bridge._
- **Why does `4. Adaptive next-question selection` connect `Community 13` to `Community 1`?**
